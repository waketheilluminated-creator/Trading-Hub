import { fetchVenueLargeTrades, parseTradesQuery } from "@/lib/market-trades.ts";
...
export async function GET(request: Request) {
  const parsed = parseTradesQuery(new URL(request.url), supportedExchangesMessage());
  if (!parsed.ok) {
    return Response.json({ error: parsed.error }, { status: 400 });
  }

  try {
    const snapshot = await fetchVenueLargeTrades(parsed.venue, parsed.symbol, parsed.minNotional);
